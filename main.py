...
ADMIN_IDS = [7662435450, 6964686224]  # Замените на ваши ID
...
REQUIRED_NAME = "@MefezyPIZZA_bot"  # Требуемая часть в отображаемом имени для активации промокода
MANUAL_LINK = "https://telegra.ph/PIZZA-MEFEZY-02-26"  # Ссылка на мануал
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str = None):
    ...
    caption = text if text else "*🍕Mefezy Pizza - Лучший сервис по отправке пиццы! Более тысячи клиентов выбирают наш сервис, только мы делаем такую вкусную,сочную пиццу.Быстрая доставка - Качественный заказ.* 🚀\n\nВыберите действие:"
...
def main() -> None:
    application = Application.builder().token("8688244720:AAEJ7hzjCiaKu7u1W9WSA-oBmT_bawf9P3U").build()
